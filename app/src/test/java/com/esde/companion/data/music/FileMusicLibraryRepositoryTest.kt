...
class FileMusicLibraryRepositoryTest {
    ...
    @Test
    fun `general pool includes root-level tracks and arbitrarily nested non-systems folders`() =
        runTest {
    ...
    @Test
    fun `general pool excludes the systems folder entirely`() =
        runTest {
    ...
    @Test
    fun `general pool ignores non-audio files`() =
        runTest {
            writeTrackFile("Lobby Theme.mp3")
    ...
    @Test
    fun `per-system pool never touches general-pool or other systems' files`() =
        runTest {
            writeTrackFile("Lobby Theme.mp3")
    ...
    @Test
    fun `per-system pool with no tracks falls back to the general pool`() =
        runTest {
