 ...
 * true - the current default - but when it's false, [appState] is [AppState.Screensaver], and
 * [visible] is true (the page is actually open on screen right now), holds [previous] instead
 * of switching to the screensaver's own game, so a user actively browsing achievements isn't
 * redirected to whatever game the screensaver happens to be showing.
 *
 * The [visible] gate matters because this resolver keeps running in the background for the
 * whole app lifetime (see [com.esde.companion.ui.retroachievements.RetroAchievementsViewModel]),
 * not just while the page is on screen: without it, "hold" would freeze [previous] at whatever
 * game was active before a screensaver started even while nobody's looking, so opening the page
 * later - while the screensaver is still going - would surface that stale held game instead of
 * the one the screensaver is actually showing right now. While [visible] is false this always
 * tracks the live [appState] regardless of the toggle, so [previous] is never stale by the time
 * the page opens - see `ObserveScreensaverAwareContextUseCase`.
 ...
 * [updateOnScreensaver] is false, but (same [visible] reasoning as [resolveAchievementsGame])
