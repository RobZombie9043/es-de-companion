 ...
 * screen with the game's manual PDF (see GameManualScreen); GameGuide replaces it with
 * the current game's most-recently-viewed downloaded guide, resumed at its last
 * position (see GameGuidesViewModel.autoOpenLastViewedGuideForCurrentGame) - both are
 * only meaningful for Game Playing Behavior, not Screensaver Behavior
 * (ScreenBehaviorPicker's `options` param excludes them from the Screensaver picker).
 */
