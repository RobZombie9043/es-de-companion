...
class FileSystemMediaRepositoryTest {
    ...
    @Test
    fun `returns null when the system has no fanart folder`() =
        runTest {
    ...
    @Test
    fun `returns the only fanart file when exactly one exists`() =
        runTest {
    ...
    @Test
    fun `picks among nested subfolder fanart and ignores non-image files`() =
        runTest {
            val nested = writeFanartFile("psx/fanart/RPGs/Final Fantasy IX (USA).jpg")
            File(mediaRoot, "psx/fanart/RPGs/readme.txt").apply {
                parentFile?.mkdirs()
                writeText("stub")
            }
            val repository = FileSystemMediaRepository(mediaRoot.absolutePath)
    ...
    @Test
    fun `only considers fanart belonging to the requested system`() =
        runTest {
    ...
    @Test
    fun `resolves media types other than fanart using their own folder`() =
        runTest {
