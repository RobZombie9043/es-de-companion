...
/**
 * Groups [GameLaunchOverrideCoordinator]'s settings-observing use cases into one parameter,
 * rather than one each, to keep its constructor within this project's LongParameterList limit -
 * same reasoning as `SelfHealConfig`/`BackupRepositories` (see CLAUDE.md).
 */
class GameLaunchOverrideSettings(
    ...
    val observeEnabled: ObserveGameLaunchEnabledUseCase,
)
