...
class ManageAppsViewModelTest {
    ...
    @Test
    fun `rows reflect every installed app with its current hidden state`() =
    ...
    @Test
    fun `toggling an app to hidden adds it to the hidden set`() =
        runTest(testDispatcher) {
    ...
    @Test
    fun `toggling an already-hidden app to visible removes it from the hidden set`() =
        runTest(testDispatcher) {
            val (viewModel, settingsRepository) =
                buildViewModel(
                    settingsRepository =
    ...
    @Test
    fun `toggling one app does not affect other hidden apps`() =
        runTest(testDispatcher) {
            val (viewModel, settingsRepository) =
                buildViewModel(
                    settingsRepository = FakeAppDrawerSettingsRepository(initialHiddenApps = setOf("com.example.b")),
