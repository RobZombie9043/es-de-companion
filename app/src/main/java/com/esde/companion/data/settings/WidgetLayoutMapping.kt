...
private fun WidgetType.toDto(): WidgetTypeDto = when (this) {
    ...
    is WidgetType.SystemImage ->
        WidgetTypeDto.SystemImage(scaleMode.toDto(), effects.blurAmount, effects.darkenAmount, panZoomEnabled, imageTransitionMode.toDto())
    ...
    is WidgetType.CustomImage ->
        WidgetTypeDto.CustomImage(path, scaleMode.toDto(), effects.blurAmount, effects.darkenAmount, panZoomEnabled, imageTransitionMode.toDto())
...
private fun WidgetTypeDto.toDomain(): WidgetType = when (this) {
    is WidgetTypeDto.SystemLogo ->
        WidgetType.SystemLogo(scaleMode.toScaleMode(), ImageEffects(blurAmount, darkenAmount), logoTransitionMode.toLogoTransitionMode(), glintEnabled)
    is WidgetTypeDto.SystemImage ->
        WidgetType.SystemImage(scaleMode.toScaleMode(), ImageEffects(blurAmount, darkenAmount), panZoomEnabled, imageTransitionMode.toImageTransitionMode())
    ...
    is WidgetTypeDto.CustomImage ->
        WidgetType.CustomImage(path, scaleMode.toScaleMode(), ImageEffects(blurAmount, darkenAmount), panZoomEnabled, imageTransitionMode.toImageTransitionMode())
