 ...
 *
 * In-memory only, same "no natural bound, not worth a disk eviction policy" reasoning as
 * [AchievementSummaryCache], with the same 15-minute TTL and `forceRefresh` bypass wired to the
 * achievement screen's kebab "Refresh" entry (refreshed alongside the achievement summary in a
 * single fetch).
 *
